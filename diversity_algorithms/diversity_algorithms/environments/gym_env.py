...
class EvaluationFunctorSwimmer:
	def __init__(self, gym_env_name=None, gym_params={}, controller=None, controller_type=None, controller_params=None, output='-dist_obj',max_step=500, bd_function=None):
		global current_serial
		#print("Eval functor created")
		#Env
		#Controller
	...
	def set_env(self, env_name, gym_params):
		...
		if(self.controller is None): # Build controller
			if(self.controller_type is None):
				raise RuntimeError("Please either give a controller or specify controller type")
			self.controller = self.controller_type(self.env.observation_space.shape[0]+2,self.env.action_space.shape[0], params=self.controller_params)
		else:
			if(self.controller_type is not None or self.controller_params is not None):
				print("WARNING: EvaluationFunctor built with both controller and controller_type/controller_params. controller_type/controller_params arguments  will be ignored")
	...
	def evaluate_indiv(self):
		...
		action_scale_factor = self.env.action_space.high # The nn generate an output in ]-1;1[ (tanh out layer); this scales to action space range
		# obs = initial_obs
		obs = np.concatenate([self.env.sim.data.qpos.flat, self.env.sim.data.qvel.flat])
		total_reward = 0.
		# Main loop
		then = time.time()
		for i in range(self.max_step):
			#self.env.render()
			action = action_scale_factor*self.controller(np.array(obs))
			_, reward, end, info = self.env.step(action) # take a random action
			obs = np.concatenate([self.env.sim.data.qpos.flat, self.env.sim.data.qvel.flat])
			# self.traj.append((obs,reward,end,info))
			self.traj.append((self.env.sim.data.qpos[:2],reward,end,info))
			total_reward += reward
			#print("Step %d : Obs=%s Action=%s Reward=%f  Dist. to objective=%f  Robot position=%s  End of ep=%s" % (i, str(obs), str(action), reward, info["dist_obj"], str(in	fo["robot_pos"]), str(end)))
			if end:
				break
...
class EvaluationFunctorBallInCup:
	def __init__(self, gym_env_name=None, gym_params={}, controller=None, controller_type=None, controller_params=None, output='-dist_obj',max_step=300, bd_function=None):
		global current_serial
		#print("Eval functor created")
		#Env
		#Controller
	...
	def set_env(self, env_name, gym_params):
		...
		if(self.controller is None): # Build controller
			if(self.controller_type is None):
				raise RuntimeError("Please either give a controller or specify controller type")
			self.controller = self.controller_type(np.concatenate([self.env.sim.data.qpos.flat, self.env.sim.data.qvel.flat]).shape[0],self.env.action_space.shape[0], params=self.controller_params)
		else:
			if(self.controller_type is not None or self.controller_params is not None):
				print("WARNING: EvaluationFunctor built with both controller and controller_type/controller_params. controller_type/controller_params arguments  will be ignored")
	...
	def evaluate_indiv(self):
		...
		for i in range(self.max_step):
			...
			# self.traj.append((obs,reward,end,info))
			# self.traj.append((self.env.sim.data.qpos[:2],reward,end,info))
			self.traj.append((self.env.ball_to_target(),reward,end,info))
			total_reward += reward
			#print("Step %d : Obs=%s Action=%s Reward=%f  Dist. to objective=%f  Robot position=%s  End of ep=%s" % (i, str(obs), str(action), reward, info["dist_obj"], str(in	fo["robot_pos"]), str(end)))
			if end:
				break
...
class EvaluationFunctorSwimmerTrajectoryBased:
	def __init__(self, gym_env_name=None, gym_params={}, controller=None, controller_type=None, controller_params=None, output='-dist_obj',max_step=100, bd_function=None):
		global current_serial
		#print("Eval functor created")
		#Env
		#Controller
	...
	def evaluate_indiv(self):
		...
		# obs = initial_obs
		obs = np.concatenate([self.env.sim.data.qpos.flat, self.env.sim.data.qvel.flat])
		# self.traj.append(copy.deepcopy(self.env.sim.data.qpos[:2]))
		...
		if len(self.traj) < self.max_step:
			for extra_t in range(len(self.traj), self.max_step):
