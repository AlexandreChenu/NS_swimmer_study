...
def visu_landscape(ind, gen, toolbox, params):
    """
    Create 100x100 images for fitness landscape visualization
    """
    resolution = 100

    fitnesses = []
    dir_indx = np.arange(0,resolution)
    ## select 100 mutation directions
    for i in range(0,resolution):
        ## compute mutation direction using a copy of the original NN
        ind_copy = copy.deepcopy(ind)

        ## mutate NN
        [mut_ind]= algorithms.varOr([ind_copy], toolbox, 1, params["cxpb"], params["mutpb"])

        ## extract mutation direction
        mut_dir = np.array(mut_ind) - np.array(ind_copy)

        ## normalize mutation direction
        norm_mut_dir = mut_dir/np.linalg.norm(mut_dir)

        alphas = list(np.linspace(-1., 1., resolution))

        # fitness = []
        # ## vary the amplitude of the mutation
        # for alpha in alphas:
        #     ## compute perturbated NN using a copy of the original NN
        #     base_ind = copy.deepcopy(ind)
        #
        #     ## perturb the NN
        #     var_ind = base_ind + alpha*norm_mut_dir ## vary ind
        #
        #     ## evaluate the NN
        #     [fit] = toolbox.map(toolbox.evaluate, [var_ind])
        #
        ...
        for alpha in alphas:
            ...
            ## perturb the NN
            var_inds.append(base_ind + alpha*norm_mut_dir) ## vary ind

        ## evaluate the perturbated NN (in parallel)
        fits= toolbox.map(toolbox.evaluate, var_inds)

        fitness = [fit[0] for fit in fits]

        fitnesses.append(fitness)

    x,y = np.meshgrid(alphas, dir_indx)
    # print("x.shape = ", x.shape)
    z = np.array(fitnesses).reshape(x.shape)
    # z.reshape(x.shape)
    # print("z.shape = ", z.shape)

    fig, ax = plt.subplots()
    mesh = ax.pcolormesh(x,y,z,vmin=0.)
    plt.colorbar(mesh, ax=ax)
    # plt.show()
    plt.savefig(params["run_name"]+"/visu_landscape_"+str(gen)+".png")
    plt.close(fig)

    return


def visu_landscape_novelty(ind, population, archive, gen, toolbox, params):
    """
    Create resolutionxresolution images for fitness novelty landscape visualization
    """
    resolution = 100
    ...
    for i in range(0,resolution):
        ...
        fake_var_inds = []
        for var_ind, fit in zip(var_inds, fits):
            fake_var_ind = toolbox.individual()
            fake_var_ind.bd=listify(fit[1])
            # print("fake_var_ind.bd = ", fake_var_ind.bd)
            fake_var_ind.novelty = archive.get_nov(fake_var_ind.bd, population)
            # print("fake_var_ind.novelty = ", fake_var_ind.novelty)
            fake_var_inds.append(fake_var_ind)

        novelty = [fake_var_ind.novelty for fake_var_ind in fake_var_inds]

        noveltys.append(novelty)

    x,y = np.meshgrid(alphas, dir_indx)
    # print("x.shape = ", x.shape)
    z = np.array(noveltys).reshape(x.shape)
    # z.reshape(x.shape)
    # print("z.shape = ", z.shape)
    ...
    plt.colorbar(mesh, ax=ax)
    # plt.show()
    plt.savefig(params["run_name"]+"/visu_novelty_landscape_"+str(gen)+".png")
    plt.close(fig)
